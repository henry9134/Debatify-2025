...
Topic.all.each do |topic|
  ...
  sides = { "for" => [], "neutral" => [], "against" => [] }

  sides.each do |status, _|
    3.times do |i|
      comment_user = users.pop
      comment = Comment.create!(
        content: "#{status.capitalize} comment #{i + 1} on topic #{topic.title}",
        status: status,
        user: comment_user,
        topic: topic
      )
      rand(0..20).times do
        vote_user = User.all.sample
        next if Vote.exists?(user: vote_user, comment: comment) # Ensure unique votes
        comment.votes.create!(user: vote_user)
      end
    end
  end
end

puts "Comments and Votes created successfully!"
